...
TPCANTimestampFD = ctypes.c_ulonglong  # Represents a timestamp of a received PCAN FD message
...
PCAN_ERROR_ANYBUSERR = TPCANStatus(PCAN_ERROR_BUSWARNING | PCAN_ERROR_BUSLIGHT | PCAN_ERROR_BUSHEAVY |
                                   PCAN_ERROR_BUSOFF | PCAN_ERROR_BUSPASSIVE)  # Mask for all bus errors
...
PCAN_CHANNEL_AVAILABLE = int(0x01)      # The PCAN-Channel handle is available to be connected \
                                        #  (Plug&Play Hardware: it means furthermore that the hardware is plugged-in)
PCAN_CHANNEL_OCCUPIED = int(0x02)       # The PCAN-Channel handle is valid, and is already being used
PCAN_CHANNEL_PCANVIEW = PCAN_CHANNEL_AVAILABLE | PCAN_CHANNEL_OCCUPIED  # The PCAN-Channel handle is already being used
                                                                        # by a PCAN-View application, but is available
                                                                        # to connect
...
PCAN_MESSAGE_BRS = TPCANMessageType(0x08)       # The PCAN message represents a FD bit rate switch
                                                # (CAN data at a higher bit rate)
PCAN_MESSAGE_ESI = TPCANMessageType(0x10)       # The PCAN message represents a FD error state indicator
                                                # (CAN FD transmitter was error active)
...
class TPCANMsg(ctypes.Structure):
    ...
    """
    _fields_ = [("ID", ctypes.c_uint),  # 11/29-bit message identifier
                ("MSGTYPE", TPCANMessageType),  # Type of the message
                ("LEN", ctypes.c_ubyte),  # Data Length Code of the message (0..8)
                ("DATA", ctypes.c_ubyte * 8)]  # Data of the message (DATA[0]..DATA[7])
...
class TPCANTimestamp(ctypes.Structure):
    ...
    """
    _fields_ = [("millis", ctypes.c_uint),  # Base-value: milliseconds: 0.. 2^32-1
                ("millis_overflow", ctypes.c_ushort),  # Roll-arounds of millis
                ("micros", ctypes.c_ushort)]  # Microseconds: 0..999
...
class TPCANMsgFD(ctypes.Structure):
    ...
    _fields_ = [("ID", ctypes.c_uint),  # 11/29-bit message identifier
                ...
                ("DATA", ctypes.c_ubyte * 64)]  # Data of the message (DATA[0]..DATA[63])
...
class PCANBasic:
    ...
    def __init__(self):
        ...
        if platform.system() == 'Windows':
            self.__m_dllBasic = ctypes.windll.LoadLibrary("PCANBasic")
        else:
    ...
    def Initialize(self, channel, baudrate_btr0btr1_register,
                   hwtype=TPCANType(0), IOPort=ctypes.c_uint(0), Interrupt=ctypes.c_ushort(0)):
        """Initializes a PCAN Channel

        Parameters:
          channel  : A TPCANHandle representing a PCAN Channel
          baudrate_btr0btr1_register : The speed for the communication (BTR0BTR1 code)
        ...
        """
        try:
            res = self.__m_dllBasic.CAN_Initialize(channel, baudrate_btr0btr1_register, hwtype, IOPort, Interrupt)
    ...
    def Read(self, channel):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_Read(channel, ctypes.byref(msg), ctypes.byref(timestamp))
    ...
    def ReadFD(self, channel):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_ReadFD(channel, ctypes.byref(msg), ctypes.byref(timestamp))
            return TPCANStatus(res), msg, timestamp
        except:
    ...
    def GetValue(self, channel, parameter):
        ...
        """
        try:
            if parameter in [PCAN_API_VERSION, PCAN_HARDWARE_NAME, PCAN_CHANNEL_VERSION, PCAN_LOG_LOCATION,
                             PCAN_TRACE_LOCATION, PCAN_BITRATE_INFO_FD, PCAN_IP_ADDRESS]:
                mybuffer = ctypes.create_string_buffer(256)
            else:
                mybuffer = ctypes.c_int(0)

            res = self.__m_dllBasic.CAN_GetValue(channel, parameter, ctypes.byref(mybuffer), ctypes.sizeof(mybuffer))
    ...
    def SetValue(self, Channel, Parameter, Buffer):
        ...
        try:
            if Parameter == PCAN_LOG_LOCATION or Parameter == PCAN_LOG_TEXT or Parameter == PCAN_TRACE_LOCATION:
                my_buffer = ctypes.create_string_buffer(256)
            else:
                my_buffer = ctypes.c_int(0)

            my_buffer.value = Buffer
            res = self.__m_dllBasic.CAN_SetValue(Channel, Parameter, ctypes.byref(my_buffer), ctypes.sizeof(my_buffer))
            return TPCANStatus(res)
        except:
    ...
    def GetErrorText(self, Error, Language=0):
        ...
        try:
            mybuffer = ctypes.create_string_buffer(256)
            res = self.__m_dllBasic.CAN_GetErrorText(Error, Language, ctypes.byref(mybuffer))
            return TPCANStatus(res), mybuffer.value
        except:
